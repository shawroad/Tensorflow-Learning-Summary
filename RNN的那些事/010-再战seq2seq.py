...
@file  : 010-再战seq2seq.py

@author: xiaolu

@time  : 2019-07-13

datasets: NLP_tensorflow_project仓库中的chatbot文件夹中含有数据
"""
import numpy as np
...
# 组织问题和回答   #　move_line中最后面括号一组对话, 一问一答的那种
convs = []
...
print('出现频率最高的6个词:', count_from[4:10])   # 这里前四个的是填充的字符
print('id和词之间的对应:', data_from[:10], [rev_dictionary_from[i] for i in data_from[:10]])
print('词表的大小', len(dictionary_from))
print("词表中词的利用比:{}".format(round(len(dictionary_from)/vocabulary_size_from, 4)*100))  # 所用到的此除以词表中所有的词
...
class Chatbot:
    def __init__(self, size_layer, num_layers, embedded_size,
        ...
        # 初始化词嵌入的向量　random_uniform()获取阶段的高斯分布随机数
        encoder_embeddings = tf.Variable(tf.random_uniform([from_dict_size, embedded_size], -1, 1))
        # decoder_embeddings = tf.Variable(tf.random_uniform([to_dict_size, embedded_size], -1, 1))
        encoder_embedded = tf.nn.embedding_lookup(encoder_embeddings, self.X)  # [[], [], []...]
        main = tf.strided_slice(self.X, [0, 0], [batch_size, -1], [1, 1])  # start=[0, 0], end=[batch_size, -1], strip=[1, 1]

        # 解码的输入 也是问题  然后进行词嵌入  fill()先去构造一个go列，然后和main数据进行按列合并　就得到我们解码的输入
        decoder_input = tf.concat([tf.fill([batch_size, 1], GO), main], 1)  # 在每一行的开始加一个GO
        decoder_embedded = tf.nn.embedding_lookup(encoder_embeddings, decoder_input)   # 将解码也及进行同样的词嵌入

        # 定义编码的模型结构 多层RNN   这里面的输入: 模型，问题的词向量, 问题的长度, 类型
        rnn_cells = tf.nn.rnn_cell.MultiRNNCell([cells() for _ in range(num_layers)])
        # 编码的过程,我们只是为了得到最终的中的状态
        ...
        # seq2seq的损失计算 logits: 代表每一步的输出, target: 代表真正的标签one_hot表示, weight: 把填充的那部分mask掉,不进行计算
        ...
        y_t = tf.cast(y_t, tf.int32)  # 应该是真是的输出

        # boolean_mask(a,b) 将使a (m维)矩阵仅保留与b中“True”元素同下标的部分
        self.prediction = tf.boolean_mask(y_t, masks)  # 也就是你的预测只能是y所在的范围内,（也就是在mask范围内）
