...
@file  : 015-model_add_checkpoint.py
...
# 保存检查点
saver = tf.train.Saver(max_to_keep=1)  # 生成saver 也就是只保存一个检查点　每步都保存，只是后一步会冲掉前一步保存的结果
savedir = "log1/"
...
#
#         # 显示训练中的详细信息
#         if epoch % display_step == 0:  # 每个一次保存一下
#             loss = sess.run(cost, feed_dict={X: train_X, Y: train_Y})
#             print("Epoch:{}, cost:{}, W:{}, b:{}".format(epoch+1, loss, sess.run(W), sess.run(b)))
#             if not (loss == "NA"):
#                 plotdata["batchsize"].append(epoch)
#                 plotdata["loss"].append(loss)
#             saver.save(sess, savedir + "linermodel.cpkt", global_step=epoch)   # 保存但前epoch的检查点
#
#     print(" Finished!")
#
